  ...
  try {
    if (where == GRB_CB_MIPSOL) {
      std::vector<std::vector<double>> sol =
          std::vector<vector<double>>(n_, std::vector<double>(n_, 0.0));
      ...
      std::vector<std::pair<double, std::pair<int, int>>> resultCapacity = arcs;
      for (int i = 0; i < m_; ++i) {
      ...
      for (int rConfig = 0; rConfig < 2; ++rConfig) {
        ...
        for (int j = 0; j < n_; ++j) {
          if (j != rFixed) {
            ...
            // SEC
            std::vector<std::pair<double, std::pair<int, int>>> capSEC;
            ...
            if (minCutSEC + UtilOptimal::kEps < n_) {
              ...
              // observe that we need just one of the two arcs {i,j} =
              // (i,j) (j,i)
              GRBLinExpr restr = 0;
              double sumS = 0;
              for (int id = 0; id < sizeS; ++id) {
                for (int jid = 0; jid < id; ++jid) {
                  if (id != jid) {
...
void SECSep::CandidateIncumbent(std::vector<vector<double>> &sol) {
  const std::vector<vector<int>> &index = internal_s_->indexes;
  const std::vector<Requirement> &require = internal_s_->requirements;
  const std::vector<std::pair<double, std::pair<int, int>>> &arcs =
      internal_s_->arcs;
