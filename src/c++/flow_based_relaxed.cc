...
Ans FlowBasedRelaxed::Solve(int timeLimit, double heuristics) {
  ...
  try {
    ...
    const double lowerBound = Solver::GetLower(model_);
    const double upperBound = Solver::GetUpper(model_);
...
void FlowBasedRelaxed::AddConstraintsAndObjective() {
  const int n = internal_s_.n;
  const int m = internal_s_.arcs.size();
  const double prob = internal_s_.prob;
  const int edges = internal_s_.edges.size();
  ...
  const std::vector<pair<double, pair<int, int> > > &arcs = internal_s_.arcs;
...
void FlowBasedRelaxed::SetInitialSolution() {
  UtilSolver::SetInitialSolutionFlow(x_, f_, NULL, &internal_s_);  //
}
